...
namespace Daxs
{
    internal sealed class HUD : DisplayConduit
    {
        ...
        public void Tick()
        {
            ...
            if (now - lastRedrawMs >= RedrawEveryMs)
            {
                lastRedrawMs = now;
        ...
        {
            if (!Enabled || RhinoDoc.ActiveDoc == null || e.Viewport.Id != RhinoDoc.ActiveDoc.Views.ActiveView.ActiveViewportID)
                return;
            ...
            {
                if (element.Enabled)
                    element.Draw(e.Display, e.Viewport, uiScale);
            }
        }
        ...
        {
            if (!textVisible) { /* hide toast if you want */ return; }
        ...
        {
            if (!textVisible) { DisableAll(); return; }
